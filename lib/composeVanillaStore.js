...
export default function composeStore(schema, definition) {
    ...
    let records = {};
    const index = [];
    const partial = validator.makePartial();
    ...
    return create((set, store) => ({
        ...
        */
        filter: (predicate) => store()
            .index
            .filter(itemIndex => predicate(store().retrieve(itemIndex))).map(matchingItemIndex => store().retrieve(matchingItemIndex)),
        /**
        ...
        remove: (idToRemove) => {
            ...
            const records = Object.assign({}, store().records);
            const oldRecord = Object.assign({}, records[idToRemove]);
        ...
        insert: (dataToAdd, optionalItemIndex) => {
            ...
            if (valid) {
                let records = Object.assign({}, store().records);
                records[itemIndex] = dataToAdd;
                if (!index.includes(itemIndex))
                    index = [...index, itemIndex];
                set({ index, records, status: "idle" });
                store().listeners.forEach(callback => callback(itemIndex, Object.assign({}, dataToAdd), "inserting"));
            }
            else {
        ...
        */
        ...
        getPartial: () => {
            return Object.assign({}, store().partial);
        },
        /**
        ...
        */
        findAndRemove: (predicate) => {
            store()
                .index
                .filter(itemIndex => predicate(store().retrieve(itemIndex))).forEach(index => {
                store().remove(index);
            });
        },
        /**
        ...
        */
        findIndex: (predicate) => store()
            .index
            .find(itemIndex => predicate(store().retrieve(itemIndex))),
        /**
...
//# sourceMappingURL=composeVanillaStore.js.map