...
import create from "zustand/vanilla"; // We should use dependency injection to inject the create function from zustand 
...
export default function composeStore(schema, definition) {
  ...
  const validator = definition ? new Validator(schema) : new Validator(schema, definition);
  let errors = [];
  /*
  ...
  let records = {};
  const index = [];
  const partial = validator.makePartial(); // Create the implementation of the store type now that we have the initial values prepared.
  ...
  return create((set, store) => ({
    ...
    */
    filter: predicate => store().index.filter(itemIndex => predicate(store().retrieve(itemIndex))).map(matchingItemIndex => store().retrieve(matchingItemIndex)),
    ...
    insert: (dataToAdd, optionalItemIndex) => {
      ...
      if (valid) {
        ...
        if (!index.includes(itemIndex)) index = [...index, itemIndex];
        set({
          index,
          records,
          status: "idle"
        });
        store().listeners.forEach(callback => callback(itemIndex, { ...dataToAdd
        }, "inserting"));
      } else {
    ...
    */
    setPartial: partialUpdate => {
      const newPartial = produce(store().partial, partialUpdate, events => {
    ...
    */
    addListener: callback => {
      set({
        listeners: [...store().listeners, callback]
      });
    },
    ...
    */
    findAndRemove: predicate => {
      store().index.filter(itemIndex => predicate(store().retrieve(itemIndex))).forEach(index => {
        store().remove(index);
      });
    },
    ...
    */
    findIndex: predicate => store().index.find(itemIndex => predicate(store().retrieve(itemIndex))),
